...
export class MainPagePage {
  // Define the isCelebrating property
  isCelebrating = false;

  // Define the hideCelebratingGif method
  hideCelebratingGif() {
  ...
  isLastTask(hobby: string): boolean {
    const index = this.selectedHobbies.indexOf(hobby);
    const lastHobby = index === this.selectedHobbies.length - 1;
    const lastTask = this.getLastTask(hobby) === this.getFirstTask(hobby);
    if (lastHobby && lastTask) {
      this.isCelebrating = true;
    }
    return lastHobby && lastTask;
  }
